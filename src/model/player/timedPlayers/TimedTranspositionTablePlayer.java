package model.player.timedPlayers;
...
import model.move.MoveComparatorEvolutionTheory;
...
public class TimedTranspositionTablePlayer extends Player {
    ...
    public TimedTranspositionTablePlayer(boolean isBlue, Board board, MoveGenerator moveGenerator, EvaluationFunction evaluationFunction, double totalTime) {
        ...
        int[] moveTypes = new int[] {1, 2, 3, 4, 5};
        int[] comp = new int[] {0, 1, 2};
        boolean frontToBack = true;
        this.maxComparator = new MoveComparatorEvolutionTheory(isEvaluationBlue(),board, maxHistoryTable, killerMoves, killerSort, moveTypes, comp, frontToBack);
        this.minComparator = new MoveComparatorEvolutionTheory(!isEvaluationBlue(),board, minHistoryTable, killerMoves, killerSort, moveTypes, comp, frontToBack);
        this.timeManager = new TimeManager(totalTime);
        this.transpositionTable= new ReportTranspositionTable();
        System.out.println("Table created..");
    }
    public TimedTranspositionTablePlayer(boolean isBlue, Board board, MoveGenerator moveGenerator, EvaluationFunction evaluationFunction, double totalTime,
                                         int window, int windowMultiplier, int interactiveDepthRatio, int fullDepthMoveNumber,
                                         float roundsFactor, float towersFactor, float distancesFactor, int[] killerSort, int[] moveTypes, int[] comp, boolean frontToBack,
                                         int peakMove, int midGameMoves, double earlyFactor, double midFactor, double endFactor) {
        super(isBlue, board, moveGenerator, evaluationFunction);
        this.window = window;
    ...
    private int maximizer(int depth, int alpha, int beta) {
        ...
        Move move;
        if (pvs != null && allMoves.contains(pvs)) move = pvs;
    ...
    private int minimizer(int depth, int alpha, int beta) {
        ...
        for (Move move : allMoves) {
            ...
            if (firstMove) {
                ...
                if (rating > alpha && rating < beta) rating = maximizer(depth - 1, alpha, beta);
            }
            zobristHashing.updateHashForMoves(move,!isEvaluationBlue);
            switchColor();
            unmakeMove(move);
